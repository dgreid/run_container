...
/// 'CGroupDirectory' represents a directory used for configuring a cgroup.
...
impl CGroupDirectory {
    pub fn new(
        ...
        let mut cg_dir = CGroupDirectory {
    ...
    pub fn chown(&self, cg_uid: Option<uid_t>, cg_gid: Option<gid_t>) -> Result<(), Error> {
        unsafe {
            // Safe because chown only reads the path from memory.
            let ret = libc::chown(
                self.path.to_string_lossy().as_ptr() as *const i8,
    ...
    pub fn add_pid(&self, pid: pid_t) -> Result<(), Error> {
        ...
        let mut tasks_file =
            fs::File::create(tasks_path.as_path()).map_err(Error::OpenTasksFile)?;
        tasks_file
    ...
    pub fn write_file(&self, name: &str, val: &str) -> Result<(), Error> {
        ...
        let mut file = fs::File::create(file_path.as_path()).map_err(Error::OpenCgroupFile)?;
