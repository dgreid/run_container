...
impl CGroupNamespace {
    ...
    pub fn join_cgroups(&mut self, pid: pid_t) -> Result<(), Error> {
        ...
        for cgroup in CGROUPS {
            try!(self.cgroup_dirs[cgroup].enter(pid));
        }
        Ok(())
    }
    ...
    pub fn enter(&self) -> Result<(), Error> {
        ...
        try!(nix::sched::unshare(CLONE_NEWCGROUP));
        Ok(())
    }
}
...
impl CGroupDir {
    pub fn new(base: &Path, parent: &Path, name: &Path, ctype: &str) -> Option<CGroupDir> {
    ...
    pub fn enter(&self, pid: pid_t) -> Result<(), Error> {
        ...
        let mut tasks_file = try!(fs::File::create(tasks_path.as_path()));
        try!(tasks_file.write_all(pid.to_string().as_bytes()));
        Ok(())
    }
}
