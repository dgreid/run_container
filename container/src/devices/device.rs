...
impl Device {
    pub fn new(
        ...
        let relative_path = PathBuf::from(path.strip_prefix("/dev/").map_err(Error::DevicePath)?);
    ...
    pub fn mknod(&self, dev_dir: &Path) -> Result<(), Error> {
        ...
        nix::sys::stat::mknod(
            ...
            Mode::from_bits(self.file_mode.unwrap_or(0)).unwrap_or_else(Mode::empty),
            nix::sys::stat::makedev(self.major.unwrap() as u64, self.minor.unwrap() as u64),
        )
        .map_err(Error::MknodFailed)?;
        let cpath = CString::new(dev_path.as_os_str().as_bytes()).map_err(Error::InvalidPath)?;
        unsafe {
            ...
            if libc::chown(
                ...
                self.gid.unwrap_or(-1i32 as libc::gid_t),
            ) < 0
            {
    ...
    pub fn bind_mount(&self, dev_dir: &Path, bind_dir: &Path) -> Result<(), Error> {
        ...
        fs::create_dir_all(node_path.parent().unwrap()).map_err(Error::CreateTargetBindPath)?;
        let orig_path = bind_dir.join(&self.path);
        unsafe {
            libc::mount(
                orig_path.to_string_lossy().as_ptr() as *const _,
                node_path.to_string_lossy().as_ptr() as *const _,
                std::ptr::null_mut(),
                MS_BIND | MS_REC,
                std::ptr::null_mut(),
            ); //TODO handle error
