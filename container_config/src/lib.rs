...
use container::cgroup::cgroup_configuration::{
    self, CGroupConfiguration, CpuAcctCGroupConfiguration, CpuCGroupConfiguration,
    CpuSetCGroupConfiguration, DevicesCGroupConfiguration, FreezerCGroupConfiguration,
};
...
use libc::uid_t;
use libc::{
    MS_BIND, MS_NOATIME, MS_NODEV, MS_NODIRATIME, MS_NOEXEC, MS_NOSUID, MS_RDONLY, MS_REC,
...
impl ContainerConfig {
    ...
    pub fn get_root_uid(&self) -> Option<uid_t> {
        self.user_namespace
            .as_ref()
            .and_then(|t| t.get_external_uid(0).map(|uid| uid as uid_t))
    }
    ...
    pub fn device_config(mut self, device_config: DeviceConfig) -> ContainerConfig {
    ...
    pub fn hook_status_template(mut self, template: Option<Box<HookState>>) -> ContainerConfig {
...
pub fn container_config_from_oci_config_file(
    path: &Path,
    ...
    let oci_config: OciConfig = serde_json::from_reader(reader).map_err(Error::ConfigParseError)?;
    container_from_oci(oci_config, bind_mounts, securebits_unlock_mask, path)
}

fn container_from_oci(
    config: OciConfig,
    bind_mounts: Vec<(String, String)>,
    securebits_unlock_mask: Option<usize>,
    path: &Path,
...
fn mount_ns_from_oci(
    mounts_vec: Option<Vec<OciMount>>,
    bind_mounts: Vec<(String, String)>,
    root_path: PathBuf,
    ...
    if let Some(mounts) = mounts_vec {
        for m in mounts {
            ...
            mnt_ns
                .add_mount(
                    Some(PathBuf::from(&m.source)),
                    PathBuf::from(&m.destination.trim_start_matches('/')),
                    Some(m.mount_type),
                    flags,
                    options,
                )
...
fn device_config_from_oci(
    dev_list: &Option<Vec<OciLinuxDevice>>,
...
fn user_ns_from_oci(
    uid_maps: &Option<Vec<OciLinuxNamespaceMapping>>,
    ...
    if let Some(ref uid_mappings) = *uid_maps {
        for id_map in uid_mappings {
            user_ns.add_uid_mapping(id_map.container_id, id_map.host_id, id_map.size);
        }
    } else {
    ...
    if let Some(ref gid_mappings) = *gid_maps {
        for id_map in gid_mappings {
            user_ns.add_gid_mapping(id_map.container_id, id_map.host_id, id_map.size);
        }
    } else {
...
fn capabilities_from_oci(
    oci_caps: OciProcessCapabilities,
    securebits_unlock_mask: Option<usize>,
) -> Result<CapConfig> {
    ...
    if let Some(ref effective) = oci_caps.effective {
        caps.set_caps(CapType::Effective, &effective)
            .map_err(Error::CreatingCapConfig)?;
    }
    if let Some(ref bounding) = oci_caps.bounding {
        caps.set_caps(CapType::Bounding, &bounding)
            .map_err(Error::CreatingCapConfig)?;
    }
    ...
    if let Some(ref ambient) = oci_caps.ambient {
        caps.set_caps(CapType::Ambient, &ambient)
            .map_err(Error::CreatingCapConfig)?;
    }
...
fn rlimits_from_oci(oci_rlim: &[OciRlimit]) -> Result<RLimits> {
    ...
    for lim in oci_rlim {
        rlimits
            .add_limit(&lim.limit_type, lim.soft as u64, lim.hard as u64)
            .map_err(Error::RLimitError)?;
    }
    Ok(rlimits)
}

fn device_cgroup_config(devices: &[OciLinuxCgroupDevice]) -> Result<Box<CGroupConfiguration>> {
    if devices.is_empty() {
    ...
    for device in devices.iter() {
        devices_config.add_device(
            ...
            device.dev_type.as_ref().map_or(Ok('a'), |t| {
                t.chars().nth(0).ok_or(Error::InvalidDeviceType)
            })?,
        )?;
    }
    Ok(devices_config)
...
fn cpuset_cgroup_config(config: &OciLinuxCgroupCpu) -> Result<Box<CpuSetCGroupConfiguration>> {
...
fn hooks_from_oci(hook_params: &[OciHook]) -> Vec<Hook> {
    ...
    for hook in hook_params.iter() {
        ...
        let new_hook = Hook::new(&PathBuf::from(&hook.path))
            ...
            .timeout(
                hook.timeout
                    .map(|sec| std::time::Duration::from_secs(sec as u64)),
            );
